...
foreach ($_SESSION['carrinho'] as $cd => $qnt)  {
    $consulta = $cn->query("SELECT vl_produto FROM tbl_produto WHERE cd_produto='$cd'");
    $exibe = $consulta->fetch(PDO::FETCH_ASSOC);
    $preco = $exibe['vl_produto'];
    
	
	$inserir = $cn->query("INSERT INTO tbl_venda(no_Ticket,cd_cliente,cd_produto,qt_produto,vl_item,dt_venda)  VALUES
